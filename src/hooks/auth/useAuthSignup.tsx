...
export const useAuthSignup = () => {
  ...
  const createAuthUser = async (email: string, password: string, fullName: string) => {
  ...
  const storeVerificationToken = async (token: string, email: string) => {
    try {
  ...
  const sendVerificationEmail = async (email: string, fullName: string, verificationUrl: string) => {
    try {
      const emailResult = await sendEmail({
        to: email,
        subject: "Welcome to Surrendered Sinner - Verify Your Email",
        html: generateEmailTemplate(fullName, verificationUrl),
      });
  ...
  const signup = async (email: string, password: string, fullName: string): Promise<SignupResult> => {
    try {
      ...
      const { data, error } = await createAuthUser(email, password, fullName);
      ...
      // Generate verification URL and store token
      const verificationUrl = generateVerificationUrl(verificationToken, email);
      console.log("Generated verification URL:", verificationUrl);
      
      await storeVerificationToken(verificationToken, email);
...
function generateEmailTemplate(fullName: string, verificationUrl: string): string {
  return `
